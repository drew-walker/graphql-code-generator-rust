...
// Parity helper kept available outside tests for potential runtime reuse.
#[allow(dead_code)]
...
fn expand_document_globs_graphql_tools_style(
    ...
    let mut paths = expand_globwalk_matches(&globs, doc_type, false)?;

    // Apply negations by filtering out matched paths
    if !ignore_negations.is_empty() {
        paths.retain(|p| {
            let path_str = p.to_string_lossy();
            !ignore_negations
                .iter()
                .any(|neg| simple_glob_matches(&path_str, neg))
        });
    }
    Ok(paths)
}
...
fn parse_executable_document(
    source: &'static str,
    location: &str,
) -> Result<QueryDocument<'static, String>> {
    match graphql_parser::parse_query::<String>(source) {
        Ok(document) => Ok(document),
        Err(parse_error) => {
            let stripped = strip_type_system_definitions_from_document(source);
            if stripped.trim() == source.trim() {
                return Err(parse_error)
                    .with_context(|| format!("failed to parse GraphQL document {location}"));
            }

            let stripped: &'static str = Box::leak(stripped.into_boxed_str());
            graphql_parser::parse_query::<String>(stripped).with_context(|| {
                format!(
                    "failed to parse GraphQL document {location} after stripping type-system definitions"
                )
            })
        }
    }
...
fn strip_type_system_definitions_from_document(source: &str) -> String {
    let mut out = String::new();
    let mut pending_prefix: Vec<&str> = Vec::new();
    let mut skipping_block: Option<usize> = None;
    ...
    for line in source.lines() {
        ...
        if let Some(brace_depth) = skipping_block.as_mut() {
            *brace_depth += count_char(line, '{');
            *brace_depth = brace_depth.saturating_sub(count_char(line, '}'));
            if *brace_depth == 0 {
                skipping_block = None;
            }
            continue;
        ...
        if let Some(block_depth) = type_system_definition_block_depth(trimmed) {
            pending_prefix.clear();
            if block_depth > 0 {
                skipping_block = Some(block_depth);
            }
            continue;
        }
...
fn type_system_definition_block_depth(trimmed: &str) -> Option<usize> {
    let rest = trimmed.strip_prefix("extend ").unwrap_or(trimmed);
    let starts_type_system = [
        "schema",
        "scalar",
        "type",
        "interface",
    ...
    .into_iter()
    .any(|keyword| starts_with_definition_keyword(rest, keyword));
    if !starts_type_system {
        return None;
    }

    let brace_depth = count_char(trimmed, '{').saturating_sub(count_char(trimmed, '}'));
    Some(brace_depth)
}

fn count_char(line: &str, needle: char) -> usize {
    line.chars().filter(|c| *c == needle).count()
}

fn starts_with_definition_keyword(line: &str, keyword: &str) -> bool {
...
/// Supports `*` (any characters except `/`), `**` (any characters including `/`), and literals.
/// Used for fast filtering without filesystem globbing on large repos.
fn simple_glob_matches(path: &str, pattern: &str) -> bool {
    // Normalize to forward slashes
    let path = path.replace('\\', "/");
    let mut pattern = pattern.replace('\\', "/").replace("./**", "");
    pattern = pattern.strip_prefix('!').unwrap_or(&pattern).to_string();

    // Handle `**/dir/**` patterns
    if pattern.contains("**/") || pattern.starts_with("*/") {
        // For `**/node_modules/**`, check if path contains `/node_modules/`
        let parts: Vec<&str> = pattern.split("**/").filter(|p| !p.is_empty()).collect();
        if parts.len() == 2 && parts[1].ends_with("/**") {
            let dir = parts[1].trim_end_matches("/**");
            return path.contains(&format!("/{}/", dir)) || path.ends_with(&format!("/{}", dir));
        }
        // For `*/...` patterns, just check if path contains the non-* parts
    ...
    // Handle `*.ext` patterns - match if filename ends with or contains the pattern
    ...
    // For other patterns, use naive suffix matching
...
async fn load_documents_for_pointers(
    ...
    // Apply excludes: filter matched files against exclude patterns without filesystem globbing.
    // Globbing exclude patterns (especially broad ones like `*.generated.tsx`) is too slow on large repos.
    if !exclude.is_empty() {
        ...
        files.retain(|p| {
            ...
            !exclude.iter().any(|ex| {
                let ex = ex.strip_prefix("./").unwrap_or(ex);
                simple_glob_matches(&path_str, ex)
            })
        });
        debug_timing(
            timing_enabled,
            format!(
                "{doc_type} applied {} exclude pattern(s), {} file(s) remain",
                exclude.len(),
                files.len()
            ),
            exclude_started,
    ...
    for path in files {
        ...
        for source in sources {
            ...
            let document = parse_executable_document(text, &path.display().to_string())?;
            debug_timing(
                timing_enabled,
    ...
    for src in inline_graphql {
        ...
        let parse_started = Instant::now();
        let document = parse_executable_document(text, "<inline>")
...
fn load_schema_graphql_files(
    ...
    debug_event(
        timing_enabled,
        format!(
            "starting multi-file SDL document merge ({} files, {} bytes)",
            files.len(),
            total_bytes
        ),
    );

    let mut merged_document = SchemaDocument {
        definitions: vec![],
    };
    for (path, raw) in files {
        let stripped = strip_schema_extensions(raw);
        let document = parse_schema::<String>(&stripped)
            .with_context(|| format!("failed to parse SDL schema {}", path.display()))?
            .into_static();
        merged_document.definitions.extend(document.definitions);
    }

    schema_generation_input_from_schema_document(&merged_document)
}
...
fn schema_generation_input_from_apollo_schema(
    ...
    let document = parse_schema::<String>(&merged_sdl)
        ...
        .into_static();
    schema_generation_input_from_schema_document(&document)
}

fn schema_generation_input_from_schema_document(
...
fn set_object_type(types: &mut BTreeMap<String, SdlType>, object: &ObjectType<'static, String>) {
    if let Some(existing) = types.get_mut(&object.name)
        && existing.kind == "OBJECT"
    {
        if existing.description.is_none() {
            existing.description = object.description.clone();
        }
        existing.fields.extend(object.fields.clone());
        merge_names(&mut existing.interfaces, &object.implements_interfaces);
        return;
    }
...
fn set_interface_type(
    ...
    if let Some(existing) = types.get_mut(&interface.name)
        && existing.kind == "INTERFACE"
    {
        if existing.description.is_none() {
            existing.description = interface.description.clone();
        }
        existing.fields.extend(interface.fields.clone());
        merge_names(&mut existing.interfaces, &interface.implements_interfaces);
        return;
    }
...
fn set_union_type(types: &mut BTreeMap<String, SdlType>, union: &UnionType<'static, String>) {
    if let Some(existing) = types.get_mut(&union.name)
        && existing.kind == "UNION"
    {
        if existing.description.is_none() {
            existing.description = union.description.clone();
        }
        merge_names(&mut existing.possible_types, &union.types);
        return;
    }
...
fn set_enum_type(types: &mut BTreeMap<String, SdlType>, enum_type: &EnumType<'static, String>) {
    if let Some(existing) = types.get_mut(&enum_type.name)
        && existing.kind == "ENUM"
    {
        if existing.description.is_none() {
            existing.description = enum_type.description.clone();
        }
        existing.enum_values.extend(enum_type.values.clone());
        return;
    }
...
fn set_input_type(types: &mut BTreeMap<String, SdlType>, input: &InputObjectType<'static, String>) {
    if let Some(existing) = types.get_mut(&input.name)
        && existing.kind == "INPUT_OBJECT"
    {
        if existing.description.is_none() {
            existing.description = input.description.clone();
        }
        existing.input_fields.extend(input.fields.clone());
        return;
    }
...
mod tests {
    use super::{
        ...
        parse_executable_document, pointer_might_be_inline_graphql,
    };
    use std::path::{Path, PathBuf};
...
"#,
    ...
    #[test]
    fn parse_executable_document_strips_leading_type_system_definitions() {
        let document = parse_executable_document(
            Box::leak(
                r#"input ChecklistTarget {
    id: ID
    name: String
}

mutation GenerateCheckItemsForCard($checklistTarget: ChecklistTarget!) {
    app {
        generateCheckItemsForCard(input: { checklistTarget: $checklistTarget }) {
            success
        }
    }
}
"#
        ...
        match &document.definitions[0] {
            graphql_parser::query::Definition::Operation(
                ...
                assert_eq!(m.variable_definitions.len(), 1);
                let selections = &m.selection_set.items;
                assert_eq!(selections.len(), 1);
                match &selections[0] {
                    graphql_parser::query::Selection::Field(field) => {
                        assert_eq!(field.name, "app");
                    }
                    other => panic!("expected top-level field selection, got {other:?}"),
                }
            }
    ...
    #[test]
    fn parse_executable_document_strips_described_type_system_definitions() {
        let document = parse_executable_document(
            Box::leak(
                r#"input ChecklistTarget {
    \"\"\"
...
"#
                .replace(
                    "input ChecklistTarget {",
                    "\"\"\"\nThe checklist target.\n\"\"\"\ninput ChecklistTarget {",
                )
                .into_boxed_str(),
            ),
            "described-mixed.graphql",
        )
        .unwrap();
    ...
    #[test]
    fn load_schema_graphql_files_merges_repeated_query_definitions_across_files() {
        let files = vec![
            (
                PathBuf::from("schema-a.graphql"),
                "type Query { a: String }\n".to_string(),
            ),
            (
                PathBuf::from("schema-b.graphql"),
                "type Query { b: Int }\n".to_string(),
            ),
        ];
        let out = load_schema_graphql_files(&files, false).expect("merged schema");
    ...
    #[test]
    fn load_schema_graphql_files_merges_repeated_mutation_definitions_across_files() {
        let files = vec![
            (
                PathBuf::from("schema-a.graphql"),
        ...
        let names: Vec<_> = mutation["fields"]
            .as_array()
            .expect("fields")
            .iter()
            .map(|f| f["name"].as_str().unwrap())
    ...
    #[test]
    fn load_schema_graphql_files_merges_repeated_object_definitions_across_files() {
        let files = vec![
            (
                PathBuf::from("schema-a.graphql"),
                "type Query { app: Application }\ntype Application { id: ID! }\n".to_string(),
            ),
            (
                PathBuf::from("schema-b.graphql"),
        ...
        let application = types
            ...
            .expect("Application type");
        let names: Vec<_> = application["fields"]
            .as_array()
            .expect("fields")
            .iter()
            .map(|f| f["name"].as_str().unwrap())
